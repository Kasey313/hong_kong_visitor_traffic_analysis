...
**Source:** Internal dataset (visitor statistics)  
...
### DAX Measures
```DAX
Total Visitor = 'dataset'[Hong Kong Residents] 
              + 'dataset'[Mainland Visitors] 
              + 'dataset'[Other Visitors]

Arrival Total = CALCULATE(
    SUM('dataset'[Total Visitor]),
    'dataset'[Arrival / Departure] = "Arrival"
)
```
